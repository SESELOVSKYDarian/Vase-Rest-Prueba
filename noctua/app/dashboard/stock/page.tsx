...
import { GestionIngredientesPanel } from '@/components/stock/GestionIngredientesPanel';

export default function StockPage() {
  const searchParams = useSearchParams();
  const [tab, setTab] = useState<'ajuste' | 'gestion'>(searchParams.get('tab') === 'gestion' ? 'gestion' : 'ajuste');
  ...
  const texts = config.theme.dashboardTexts?.stock || { title: 'Inventario', subtitle: 'Gestiona el inventario de ingredientes' };
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'ajuste' ? (
        ...
        <GestionIngredientesPanel isLoading={isLoading} />
      )}
    </div>
  );
}
