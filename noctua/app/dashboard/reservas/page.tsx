...
function ReservaCard({
  ...
  return (
    <motion.div
      ...
      className="bg-canvas border border-line rounded-xl overflow-hidden"
    >
      {/* Header */}
      <button
      ...
      >
        ...
        <div className="flex-shrink-0">
          <p className="text-[10px] text-ink-3 tracking-widest uppercase mb-0.5">
            Código
          </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                {reserva.email_cliente && (
                  <div className="flex items-center gap-2">
                    <Mail size={12} className="text-ink-3 flex-shrink-0" />
                ...
                {reserva.telefono && (
                  <div className="flex items-center gap-2">
                    <Phone size={12} className="text-ink-3 flex-shrink-0" />
              ...
              {reserva.estado === 'activa' && (
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-700 dark:text-emerald-400 text-xs font-semibold hover:bg-emerald-500/20 transition-colors"
                  >
                    <CheckCircle size={12} />
                    Completar
                  </button>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 border border-red-500/20 rounded-lg text-red-700 dark:text-red-400 text-xs font-semibold hover:bg-red-500/20 transition-colors"
                  >
                    <XCircle size={12} />
                    Cancelar
                  </button>
  ...
  return (
    <div className="flex flex-col gap-5 h-[calc(100vh-8rem)]">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        <div>
          <h1 className="font-display text-2xl font-semibold text-ink tracking-widest uppercase">
            Reservas
          </h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-surface border border-line rounded-xl text-sm text-ink-2 hover:border-line-strong hover:text-ink transition-all disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-3 gap-3 flex-shrink-0">
        {[
          { label: 'Activas', value: activas, color: 'text-emerald-700 dark:text-emerald-400' },
          { label: 'Completadas', value: completadas, color: 'text-sky-700 dark:text-sky-400' },
          { label: 'Canceladas', value: canceladas, color: 'text-red-700 dark:text-red-400' },
        ].map((s) => (
          <div
            key={s.label}
            className="bg-canvas border border-line rounded-xl p-4"
          >
            <p className="text-xs text-ink-3 tracking-widest uppercase mb-1">
              {s.label}
            </p>
            <p className={cn('text-3xl font-semibold font-mono', s.color)}>
              {s.value}
            </p>
          </div>
        ))}
      ...
      <div className="flex items-center gap-3 flex-shrink-0 flex-wrap">
        {/* Tabs */}
        <div className="flex gap-1 bg-canvas border border-line rounded-xl p-1">
          {FILTROS.map((f) => (
            <button
              key={f.value}
            ...
            >
              ...
              <span
                className={cn(
                  'text-[10px] font-mono',
                  filtro === f.value ? 'text-black/60' : 'text-ink-3'
                )}
              >
                {f.count}
              </span>
        ...
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input
            ...
            className="w-full bg-canvas border border-line rounded-xl pl-8 pr-3 py-2 text-sm text-ink placeholder:text-ink-3 focus:outline-none focus:border-line-strong transition-colors"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-40 gap-3">
            <RefreshCw size={24} className="text-ink-3 animate-spin" />
            <p className="text-ink-3 text-sm">Cargando reservas…</p>
          </div>
        ) : error ? (
          ...
          <EmptyState title="Sin reservas para mostrar" hint="Las reservas nuevas aparecen acá apenas se cargan." />
        ) : (
