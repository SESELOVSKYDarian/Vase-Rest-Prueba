...
export function Sidebar() {
  ...
  return <>
    <button className="lg:hidden fixed top-5 left-4 z-[60] h-11 w-11 flex items-center justify-center bg-[#151a16] border border-[#2b3a2f] rounded-xl text-white shadow-lg shadow-black/30" onClick={toggleMobileNav} aria-label="Abrir menú">{mobileOpen ? <X size={20} /> : <Menu size={20} />}</button>
    <AnimatePresence>{mobileOpen && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="lg:hidden fixed inset-0 bg-black/60 z-40" onClick={closeMobileNav} />}</AnimatePresence>
    <aside className={cn('fixed left-0 top-0 h-full w-20 hover:w-72 focus-within:w-72 bg-[#0e0e0e]/90 backdrop-blur-xl border-r border-[#1d2b21] flex flex-col z-50 transition-all duration-300 overflow-hidden group', mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
      ...
      <nav className="flex-1 px-3 py-8 space-y-5 overflow-y-auto" role="navigation" aria-label="Navegación principal">
        <div className="space-y-3">
          <Link href="/dashboard" title="Inicio" onClick={closeMobileNav} aria-current={pathname === '/dashboard' ? 'page' : undefined} className={cn('flex items-center justify-start gap-4 px-4 py-4 rounded-[22px] text-sm font-medium transition-all duration-300 min-w-[240px]', pathname === '/dashboard' ? 'bg-[#7ed957]/12 text-[#b7f397] shadow-[inset_0_0_0_1px_rgba(126,217,87,0.18)]' : 'text-[#829487] hover:text-white hover:bg-[#7ed957]/7')}>
          ...
          <div key={categoria} role="group" aria-label={LABEL_POR_CATEGORIA[categoria]} className="space-y-1">
            ...
            <div className="space-y-2">
              {items.map(({ seccion, icon: Icon }) => {
                ...
                return <Link key={href} href={href} title={LABEL_POR_SECCION[seccion]} onClick={closeMobileNav} aria-current={isActive ? 'page' : undefined} className={cn('flex items-center justify-start gap-4 px-4 py-4 rounded-[22px] text-sm font-medium transition-all duration-300 min-w-[240px]', isActive ? 'bg-[#7ed957]/12 text-[#b7f397] shadow-[inset_0_0_0_1px_rgba(126,217,87,0.18)]' : 'text-[#829487] hover:text-white hover:bg-[#7ed957]/7')}>
