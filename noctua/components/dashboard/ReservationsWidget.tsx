...
export function ReservationsWidget({ data, loading, error }: ReservationsWidgetProps) {
  ...
  return (
    <section className="rounded-2xl border border-line bg-surface p-7">
      <h2 className="text-base text-ink text-ink-2 font-medium">Reservas</h2>
      ...
      <div className="mt-6">
        <div className="mb-2 flex items-center justify-between text-sm">
          <span className="text-ink-2">Tasa de cancelación</span>
          <span className="font-semibold text-ink">{data.cancelRate.toFixed(1)}%</span>
        </div>
...
function Metric({
  ...
  return (
    <div className="rounded-md border border-line bg-white/[0.02] p-3">
      <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-ink-3">
        {Icon && <Icon size={14} />}
        {label}
      </div>
      <p className={tone === 'green' ? 'mt-2 text-2xl font-bold text-green-700 dark:text-green-400' : tone === 'red' ? 'mt-2 text-2xl font-bold text-red-700 dark:text-red-400' : 'mt-2 text-2xl font-bold text-ink'}>
        {value}
      </p>
    </div>
  );
