...
export function DeliveryAppLogo({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return <span aria-hidden="true" className={`flex shrink-0 items-center justify-center overflow-hidden shadow-sm ${dimensions}`} style={{ backgroundColor: colors.background, color: colors.foreground }}>
    {id === 'pedidosya' && <span className="text-lg font-semibold italic tracking-[-0.15em]">P!</span>}
    {id === 'rappi' && <span className="text-xl font-semibold italic">R</span>}
    {id === 'ubereats' && <span className="flex flex-col text-center text-[8px] font-semibold leading-[0.9]"><span>UBER</span><span className="text-[#06c167]">EATS</span></span>}
